...
class TestAuthEnabled:
    ...
    # --- Intelligence endpoints (explicitly protected) ---

    def test_intelligence_loss_analysis_no_key(self):
        assert client.get("/internal/intelligence/loss-analysis").status_code == 401

    def test_intelligence_loss_analysis_wrong_key(self):
        assert client.get("/internal/intelligence/loss-analysis", headers={"X-API-Key": "wrong"}).status_code == 403

    def test_intelligence_loss_analysis_with_key(self):
        assert client.get("/internal/intelligence/loss-analysis", headers={"X-API-Key": TEST_API_KEY}).status_code == 200

    def test_intelligence_score_effectiveness_no_key(self):
        assert client.get("/internal/intelligence/score-effectiveness").status_code == 401

    def test_intelligence_score_effectiveness_wrong_key(self):
        assert client.get("/internal/intelligence/score-effectiveness", headers={"X-API-Key": "wrong"}).status_code == 403

    def test_intelligence_score_effectiveness_with_key(self):
        assert client.get("/internal/intelligence/score-effectiveness", headers={"X-API-Key": TEST_API_KEY}).status_code == 200

    def test_intelligence_cohorts_no_key(self):
        assert client.get("/internal/intelligence/cohorts").status_code == 401

    def test_intelligence_cohorts_wrong_key(self):
        assert client.get("/internal/intelligence/cohorts", headers={"X-API-Key": "wrong"}).status_code == 403

    def test_intelligence_cohorts_with_key(self):
        assert client.get("/internal/intelligence/cohorts", headers={"X-API-Key": TEST_API_KEY}).status_code == 200

    # --- Outcome history endpoint (explicitly protected) ---

    def test_outcome_history_no_key(self):
        assert client.get("/internal/outcomes/history/1").status_code == 401

    def test_outcome_history_wrong_key(self):
        assert client.get(
            "/internal/outcomes/history/1",
            headers={"X-API-Key": "wrong"},
        ).status_code == 403

    def test_outcome_history_with_key(self):
        r = client.get(
            "/internal/outcomes/history/1",
            headers={"X-API-Key": TEST_API_KEY},
        )
