...
def post_outcome(body: OutcomeRequest) -> OutcomeResponse:
    """Record or update the real-world outcome for a lead.

    Backward compatible, idempotent, transactional.
    - loss_reason only accepted when outcome='lost'; silently set to NULL otherwise.
    - recorded_by does NOT participate in idempotency check.
    - Idempotency: if (outcome, loss_reason, reason, notes) match current snapshot,
      no history row is created and recorded_at is not updated.
    """
    db = get_db()
    ...
    # Enforce loss_reason rules
    loss_reason = body.loss_reason if body.outcome == "lost" else None

    # Idempotency check: compare (outcome, loss_reason, reason, notes) only
    current = db.execute(
        "SELECT outcome, loss_reason, reason, notes, recorded_by, recorded_at "
    ...
    if current is not None:
        is_duplicate = (
            ...
            and current["loss_reason"] == loss_reason
            and current["reason"] == body.reason
            and current["notes"] == body.notes
        )
        if is_duplicate:
            return OutcomeResponse(
                ...
                notes=current["notes"],
                loss_reason=current["loss_reason"],
                recorded_by=current["recorded_by"] or "system",
                recorded_at=current["recorded_at"],
            )

    now = datetime.now(timezone.utc).isoformat()

    # Transactional: upsert lead_outcomes + append lead_outcome_history.
    # Explicit rollback on failure prevents partial state on the shared connection.
    try:
        db.execute(
            """
            INSERT INTO lead_outcomes (lead_id, outcome, reason, notes, loss_reason, recorded_by, recorded_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            ...
            """,
            (body.lead_id, body.outcome, body.reason, body.notes, loss_reason, body.recorded_by, now),
        )
        db.execute(
            """
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (body.lead_id, body.outcome, loss_reason, body.reason, body.notes, body.recorded_by, now),
        )
        db.commit()
    except Exception:
        db.rollback()
    ...
    emit_event(
        ...
        {"outcome": body.outcome, "loss_reason": loss_reason},
    )
    return OutcomeResponse(
        lead_id=body.lead_id,
...
@router.get(
    "/internal/outcomes/history/{lead_id}",
    response_model=OutcomeHistoryResponse,
    dependencies=[Depends(require_api_key)],
)
def get_outcome_history(lead_id: int) -> OutcomeHistoryResponse:
    ...
    history_rows = db.execute(
        "SELECT outcome, loss_reason, reason, notes, recorded_by, recorded_at "
        "FROM lead_outcome_history WHERE lead_id = ? ORDER BY id ASC",
        (lead_id,),
    ).fetchall()

    current = None
    if current_row:
        current = OutcomeHistoryEntry(
            outcome=current_row["outcome"],
            loss_reason=current_row["loss_reason"],
            reason=current_row["reason"],
            notes=current_row["notes"],
            recorded_by=current_row["recorded_by"] or "system",
...
_RECENTLY_CONTACTED_HOURS = 72


@router.post(
    "/internal/contact-attempts",
    response_model=ContactAttemptResponse,
    status_code=201,
    dependencies=[Depends(require_api_key)],
)
def post_contact_attempt(body: ContactAttemptRequest) -> ContactAttemptResponse:
    """Record an outbound contact attempt against a lead.

    Append-only — every POST creates a new row. Does not mutate lead_outcomes.
    Event emission is best-effort after successful commit.
    """
    db = get_db()
    lead = db.execute("SELECT id FROM leads WHERE id = ?", (body.lead_id,)).fetchone()
    if lead is None:
    ...
    now = datetime.now(timezone.utc).strftime(_SQLITE_DT_FMT)

    try:
        db.execute(
            """
            INSERT INTO contact_attempts
                (lead_id, channel, direction, attempt_type, status, provider, note, external_ref, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (body.lead_id, body.channel, body.direction, body.attempt_type,
             body.status, body.provider, body.note, body.external_ref, now),
        )
    ...
    emit_event(
        "contact_attempt.recorded", "lead", body.lead_id, "contact_attempts",
        {"channel": body.channel, "attempt_type": body.attempt_type,
         "status": body.status, "provider": body.provider},
    )

    return ContactAttemptResponse(
        id=row_id, lead_id=body.lead_id, channel=body.channel,
        direction=body.direction, attempt_type=body.attempt_type,
        status=body.status, provider=body.provider,
        note=body.note, external_ref=body.external_ref, created_at=now,
    )


@router.get(
    "/internal/contact-attempts/{lead_id}",
    response_model=ContactAttemptSummaryResponse,
    dependencies=[Depends(require_api_key)],
)
def get_contact_attempts(lead_id: int) -> ContactAttemptSummaryResponse:
    """Recent contact attempts for a lead, newest first. Capped at 50."""
    db = get_db()
    lead = db.execute("SELECT id FROM leads WHERE id = ?", (lead_id,)).fetchone()
    if lead is None:
        raise HTTPException(status_code=404, detail=f"Lead {lead_id} not found")
    ...
    total_row = db.execute(
        "SELECT COUNT(*) FROM contact_attempts WHERE lead_id = ?", (lead_id,),
    ).fetchone()
    attempt_count = total_row[0]

    rows = db.execute(
        """
        SELECT id, lead_id, channel, direction, attempt_type, status,
               provider, note, external_ref, created_at
    ...
    last_contacted_at = rows[0]["created_at"] if rows else None
    last_attempt_status = rows[0]["status"] if rows else None
    last_channel = rows[0]["channel"] if rows else None

    return ContactAttemptSummaryResponse(
        lead_id=lead_id,
        ...
        recent_attempts=[
            ContactAttemptResponse(
                id=row["id"], lead_id=row["lead_id"], channel=row["channel"],
                direction=row["direction"], attempt_type=row["attempt_type"],
                status=row["status"], provider=row["provider"],
                note=row["note"], external_ref=row["external_ref"],
                created_at=row["created_at"],
            )
            for row in rows
        ],
...
def _get_contact_attempt_enrichment(lead_ids: list[int]) -> dict[int, dict]:
    """Return {lead_id: {last_contacted_at, contact_attempts_count, recently_contacted}}.

    Only counts outbound attempts. Leads with no attempts are absent from the result.
    """
    if not lead_ids:
        return {}
    db = get_db()
    ...
    rows = db.execute(
        ...
        SELECT lead_id,
               ...
               COUNT(*) AS contact_attempts_count
        FROM contact_attempts
    ...
    cutoff = (datetime.now(timezone.utc) - timedelta(hours=_RECENTLY_CONTACTED_HOURS)).strftime(_SQLITE_DT_FMT)
    return {
        row["lead_id"]: {
            "last_contacted_at": row["last_contacted_at"],
            "contact_attempts_count": row["contact_attempts_count"],
            "recently_contacted": row["last_contacted_at"] >= cutoff,
        }
        for row in rows
    }
...
def get_followup_queue() -> FollowupQueueResponse:
    ...
    claimed_ids = _get_claimed_lead_ids()
    eligible = [row for row in rows if row["lead_id"] not in claimed_ids]
    enrichment = _get_contact_attempt_enrichment([r["lead_id"] for r in eligible])
    items: list[FollowupItem] = []
    for row in eligible:
        pack = get_lead_pack(row["lead_id"])
        e = enrichment.get(row["lead_id"], {})
        items.append(FollowupItem(
            ...
            last_contacted_at=e.get("last_contacted_at"),
            contact_attempts_count=e.get("contact_attempts_count", 0),
            recently_contacted=e.get("recently_contacted", False),
        ))
...
def get_daily_actions() -> DailyActionsResponse:
    ...
    top_slice = all_followup[:_DAILY_CAP]
    followup_enrichment = _get_contact_attempt_enrichment([r["lead_id"] for r in top_slice])
    top_followup = [
        DailyFollowupItem(
            lead_id=r["lead_id"],
            name=r["name"],
            ...
            last_contacted_at=followup_enrichment.get(r["lead_id"], {}).get("last_contacted_at"),
            contact_attempts_count=followup_enrichment.get(r["lead_id"], {}).get("contact_attempts_count", 0),
            recently_contacted=followup_enrichment.get(r["lead_id"], {}).get("recently_contacted", False),
        )
        for r in top_slice
    ]
...
def get_followup_automation() -> FollowupAutomationResponse:
    ...
    claimed_ids = _get_claimed_lead_ids()
    eligible = [r for r in rows if r["lead_id"] not in claimed_ids]
    enrichment = _get_contact_attempt_enrichment([r["lead_id"] for r in eligible])
