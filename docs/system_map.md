...
| Contact Attempts (write) | Endpoint | `apps/api/routes/internal.py` → `POST /internal/contact-attempts` | Record outbound contact attempt against a lead. Append-only, auth required |
| Contact Attempts (read) | Endpoint | `apps/api/routes/internal.py` → `GET /internal/contact-attempts/{lead_id}` | Recent contact attempts summary for a lead. Auth required |
| `contact_attempts` | Table | `apps/api/db.py` | Append-only outbound contact attempt log. Indexed by `(lead_id, created_at DESC)` |
| `_get_contact_attempt_enrichment()` | Private helper (acts as shared construct) | `apps/api/routes/internal.py` | Contact attempt enrichment for followup surfaces — used by 4 endpoints |
| Daily Ops Snapshot | n8n workflow | `deploy/n8n/daily-ops-snapshot.json` | First n8n workflow — sends daily Telegram snapshot consuming `/internal/daily-actions` + `/internal/ops/snapshot` |
...
| `POST /internal/contact-attempts` | "Record that a contact attempt happened" — append-only, auth required |
| `GET /internal/contact-attempts/{lead_id}` | "What contact attempts exist for this lead?" — summary + recent list |
