...
**Depends on:** `_get_actionable_leads()`, `_get_claimed_lead_ids()`, `get_lead_pack()`, `get_rating()`, `_get_contact_attempt_enrichment()`
**Used by:** Followup bridge, daily-actions
**Source of truth:** `docs/operational_contracts.md` → followup sections
**Key tests:** `test_api.py` — shape, fields, exclusions, rating, ordering, determinism. `test_contact_attempts.py` — enrichment tests.
**Notes:** Helpers `_followup_instruction(rating)` and `_followup_message(name, rating)` in `internal.py`. All 4 surfaces enriched with `last_contacted_at`, `contact_attempts_count`, `recently_contacted` from contact_attempts table.

---

## Contact Attempts

**Type:** Endpoint group (write + read)
**Status:** Active
**Primary path:** `apps/api/routes/internal.py`
**Purpose:** Record and retrieve outbound contact attempts against leads. Provides enrichment data to followup surfaces.
**Depends on:** `leads` table (FK), `emit_event()`, `require_api_key`
**Used by:** followup-queue, followup-handoffs, followup-automation, daily-actions (enrichment via `_get_contact_attempt_enrichment()`)
**Source of truth:** `docs/operational_contracts.md` → contact attempts sections
**Auth:** Both endpoints require `require_api_key`
**Key tests:** `tests/api/test_contact_attempts.py` — POST/GET happy path, validation, enrichment, non-regression
**Notes:** Append-only. `created_at` stored in SQLite-friendly format (`YYYY-MM-DD HH:MM:SS` UTC). Transactional write with explicit rollback. Event emission (`contact_attempt.recorded`) is best-effort after successful commit. Cooldown marker `recently_contacted` uses 72-hour window. Does not mutate `lead_outcomes`.
...
## Daily Ops Snapshot (n8n)

**Type:** n8n workflow (read-only consumer)
**Status:** Active — first n8n workflow in the project
**Primary path:** `deploy/n8n/daily-ops-snapshot.json`
**Purpose:** Send a daily actionable Telegram message to the operator summarizing pending work
**Depends on:** `GET /internal/daily-actions`, `GET /internal/ops/snapshot`
**Used by:** Operator (via Telegram)
**Source of truth:** `docs/n8n_daily_ops_runbook.md`
**Notes:** Zero blast radius on core. Read-only. No business logic in n8n — all prioritization comes from OpenClaw. Configurable timezone (default `Europe/Madrid`), auth via `X-API-Key` header from env vars. Includes "HOY PRIMERO" section (max 5 items: urgent reviews, client-ready, stale follow-ups).

---

## New Lead Notification (n8n)

**Type:** n8n workflow (read-only consumer)
**Status:** Built, pending activation — second n8n workflow in the project
**Primary path:** `deploy/n8n/new-lead-notification.json`
**Purpose:** Poll for new leads every 5 minutes, send Telegram notification per lead
**Depends on:** `GET /leads?limit=N&offset=N`, `GET /leads/{id}/pack`
**Used by:** Operator (via Telegram)
**Source of truth:** `docs/n8n_new_lead_runbook.md`
**Notes:** Zero blast radius on core. Read-only. n8n-side watermark via static data (`lastSeenId`). Paginated detection with in-memory dedup. First-run seeds without sending. Alert prefix for urgent leads (uses `pack.alert` from OpenClaw). N+1 pack fetch accepted as MVP trade-off. Same Telegram bot as daily-ops-snapshot.

---

## Follow-up Action Digest (n8n)

**Type:** n8n workflow (read-only consumer)
**Status:** Built, pending activation — third n8n workflow in the project
**Primary path:** `deploy/n8n/followup-digest.json`
**Purpose:** Send an afternoon Telegram digest of actionable follow-up leads with copy-paste suggested messages
**Depends on:** `GET /internal/followup-automation`
**Used by:** Operator (via Telegram)
**Source of truth:** `docs/n8n_followup_digest_runbook.md`
**Notes:** Zero blast radius on core. Read-only. Stateless (no watermark). Filters `recently_contacted=false` to show only actionable leads. Groups by rating tier (high/medium/low). Includes suggested messages for operator copy-paste. Max 5 detailed leads, overflow shown as `+ N más`. Weekend guard in Code node. Complements daily-ops-snapshot (09:00 morning overview) with afternoon follow-up focus. Same Telegram bot as other workflows.

---

## Sentinel Health Alert (n8n)

**Type:** n8n workflow (read-only consumer)
**Status:** Built, pending activation — fourth n8n workflow in the project
**Primary path:** `deploy/n8n/sentinel-health-alert.json`
**Purpose:** Poll sentinel every 15 minutes, alert operator on health degradation via Telegram
**Depends on:** `GET /internal/sentinel`
**Used by:** Operator (via Telegram)
**Source of truth:** `docs/n8n_sentinel_health_runbook.md`
**Notes:** Zero blast radius on core. Read-only. Fingerprint-based noise control: `status|sorted_check_names`. Alerts on fingerprint change (new or different problem). 6-hour cooldown for unchanged persistent issues. Recovery message when health returns to ok. Covers all 3 sentinel checks (stale_claims, source_needs_attention, event_spine_silent). Auto-includes future sentinel checks in fingerprint. Same Telegram bot as other workflows.
...
**Used by:** Nothing automated (consumed by operator manually; n8n workflow built but not yet deployed)
...
**Used by:** n8n daily-ops-snapshot (deployed), operator manual queries
**Source of truth:** `docs/operational_contracts.md`
...
**Depends on:** `_get_actionable_leads()`, `_get_claimed_lead_ids()`
**Used by:** n8n daily-ops-snapshot (deployed), audit (ops_snapshot_arithmetic check)
**Source of truth:** `docs/operational_contracts.md`
