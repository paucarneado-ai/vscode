...
| /internal/contact-attempts | POST | OK — record an outbound contact attempt against a lead. Append-only, transactional write with explicit rollback. Emits `contact_attempt.recorded` event (best-effort after commit). Auth: `require_api_key` |
| /internal/contact-attempts/{lead_id} | GET | OK — recent contact attempts for a lead. Returns summary (count, last status/channel/timestamp) + up to 50 most recent attempts. Auth: `require_api_key` |
...
| lead.outcome_recorded | lead | outcomes | `{"outcome": "<value>", "loss_reason": "<value or null>"}` | Outcome recorded or updated via POST /internal/outcomes |
...
Records or updates the real-world outcome for a lead. Upsert semantics — one outcome per lead, latest write wins. Backward compatible, idempotent, transactional.
...
| reason | string | no | null | Free text explaining why this outcome |
| notes | string | no | null | Optional additional context |
| loss_reason | string | no | null | Only accepted when outcome='lost'. One of: price, timing, competitor, no_need, no_response, other. Silently set to null if outcome != 'lost'. |
| recorded_by | string | no | "system" | Identifier of who recorded this outcome. Does NOT participate in idempotency. |
...
| loss_reason | string or null |
| recorded_by | string |
| recorded_at | ISO 8601 timestamp |

**Errors:** 404 if lead_id does not exist. 422 if outcome is not in allowed set, if loss_reason is invalid when outcome='lost', or if lead_id is missing.

**Idempotency:** Compares `(outcome, loss_reason, reason, notes)` against current snapshot. If all 4 match → no-op: no history row created, recorded_at and recorded_by unchanged, returns current row. `recorded_by` is excluded from idempotency comparison.

**Transaction:** Upsert into `lead_outcomes` + insert into `lead_outcome_history` happen in a single commit. If either fails, neither persists.

**Persistence:** `lead_outcomes` table (current snapshot, PK = lead_id) + `lead_outcome_history` table (append-only log of every change).

### GET /internal/outcomes/history/{lead_id}

Full outcome history for a lead, ordered chronologically.
...
|---|---|
| lead_id | integer |
| current | OutcomeHistoryEntry or null (null if no outcome recorded) |
| history | list of OutcomeHistoryEntry, ordered by insertion (ASC) |
...
**Errors:** 404 if lead_id does not exist in `leads` table.

**Backfill note:** Pre-existing outcomes (before history table was created) have a single synthetic backfill row with `recorded_by = "backfill"`. This does NOT reconstruct real prior history — it preserves only the snapshot that existed at migration time.

### GET /internal/intelligence/loss-analysis

Distribution of loss reasons across lost leads. Protected by API key.

**Query params:** `source` (optional, exact match).
...
| total_lost | integer | Total leads with outcome='lost' |
| by_reason | object | Keys: price, timing, competitor, no_need, no_response, other, unspecified. Values: integer counts. |
| top_reason | string or null | Highest count reason, excluding 'unspecified'. Null if all zero. |
| by_source | list | Per-source breakdown. Only sources with ≥1 lost lead. Ordered by total_lost DESC. |

**by_source item:** `{ source, total_lost, top_reason, reasons: {same keys as by_reason} }`

**`unspecified`** = outcome='lost' but loss_reason IS NULL.

### GET /internal/intelligence/score-effectiveness

Score vs outcome correlation by score buckets. Protected by API key.
...
| generated_at | ISO 8601 timestamp | |
| source_filter | string or null | |
| total_with_outcomes | integer | Total leads with any outcome |
| buckets | list of 4 | Ranges: 0-30, 31-50, 51-70, 71-100 |
| scoring_accuracy_signal | string | "reliable", "weak", or "insufficient_data" |

**Bucket fields:**

| Field | Source | Formula |
|---|---|---|
| total | current snapshot | Leads in bucket with any outcome |
| contacted_or_beyond | **history** | Lead ever had outcome in: contacted, qualified, won, lost, bad_fit, no_answer |
| qualified_or_beyond | **history** | Lead ever had outcome in: qualified, won, lost, bad_fit |
| won | current snapshot | outcome='won' |
| lost | current snapshot | outcome='lost' |
| bad_fit | current snapshot | outcome='bad_fit' |
| no_answer | current snapshot | outcome='no_answer' |
| contact_rate | derived | contacted_or_beyond / total (0 if total=0) |
| qualification_rate | derived | qualified_or_beyond / total (0 if total=0) |
| win_rate_on_terminal | derived | won / (won+lost+bad_fit) (0 if denominator=0) |

**scoring_accuracy_signal:** `insufficient_data` if total_with_outcomes < 10. Otherwise: `reliable` if bucket 71-100 win_rate_on_terminal > bucket 0-30 win_rate_on_terminal, else `weak`.

### GET /internal/intelligence/cohorts

Monthly cohort analysis based on lead creation month. Protected by API key.

**Query params:** `months` (integer 1-12, default 3), `source` (optional, exact match).
...
| cohorts | list | One item per month, ascending chronological order |

**Cohort item fields:**

| Field | Source | Notes |
|---|---|---|
| month | leads.created_at | Format: "YYYY-MM" |
| leads_created | count | All leads created in this month |
| with_outcomes | count | Leads with any recorded outcome |
| contacted_or_beyond | **history** | Same definition as score-effectiveness |
| qualified_or_beyond | **history** | Same definition as score-effectiveness |
| won | current snapshot | |
| lost | current snapshot | |
| bad_fit | current snapshot | |
| no_answer | current snapshot | |
| avg_score | derived | Average score of all leads in cohort (not just those with outcomes) |
| contact_rate | derived | contacted_or_beyond / with_outcomes (0 if 0) |
| qualification_rate | derived | qualified_or_beyond / with_outcomes (0 if 0) |
| win_rate_on_terminal | derived | won / (won+lost+bad_fit) (0 if denominator=0) |

Empty months (0 leads created) are included with all values = 0.
...
### POST /internal/contact-attempts

**Purpose:** Record an outbound contact attempt against a lead. Append-only — every POST creates a new row. Does not mutate `lead_outcomes`.
...
|-------|------|----------|--------|
| lead_id | int | yes | Must reference existing lead |
| channel | string | yes | `email`, `phone`, `whatsapp`, `other` |
| direction | string | no (default: `outbound`) | `outbound` (MVP-only) |
| attempt_type | string | yes | `first_contact`, `follow_up`, `re_engage` |
| status | string | yes | `sent`, `answered`, `no_answer`, `failed`, `bounced` |
| provider | string | no (default: `manual`) | `manual`, `n8n`, `api`, `other` |
| note | string | no | Free-form note |
| external_ref | string | no | External reference (e.g., n8n job ID) |
...
**Write path:** Transactional write with explicit rollback on failure. Event emission (`contact_attempt.recorded`) is best-effort after successful commit — if event emission fails, the contact attempt is still persisted.

**Behavior:**
- Lead must exist (404 if not).
- Enum validation via Pydantic Literal types (422 on invalid values).
- `created_at` set by application in SQLite-friendly format (`YYYY-MM-DD HH:MM:SS` UTC).
- No idempotency — every POST creates a new row.
- Does not affect `lead_outcomes` or any other table.

**Known debt:**
- No rate limiting.
- No batch write.
- `direction` is always `outbound` — field exists for future extensibility.

### GET /internal/contact-attempts/{lead_id}

**Purpose:** Recent contact attempts for a lead, newest first.
...
|-------|------|
| lead_id | int |
| attempt_count | int (total, not capped) |
| last_contacted_at | string or null |
| last_attempt_status | string or null |
| last_channel | string or null |
| recent_attempts | list of ContactAttemptResponse (capped at 50) |

**Behavior:**
- Lead must exist (404 if not).
- `recent_attempts` ordered by `created_at DESC, id DESC` (stable tiebreaker).
- `last_contacted_at`, `last_attempt_status`, `last_channel` derived from the most recent attempt.
- `attempt_count` reflects total count, not capped.

**Known debt:**
- No pagination.
- Cap is hardcoded at 50.

### Followup surface enrichment (contact attempts)

The following surfaces now include contact attempt enrichment fields on each item:

- `GET /internal/followup-queue` — on each `FollowupItem`
- `GET /internal/followup-handoffs` — on each `FollowupHandoffItem`
- `GET /internal/followup-automation` — inside each item's `payload` (FollowupAutomationPayload)
- `GET /internal/daily-actions` — on each `DailyFollowupItem` in `top_followup`

**Enrichment fields (additive, all have defaults):**

| Field | Type | Default | Meaning |
|-------|------|---------|---------|
| last_contacted_at | string or null | null | Timestamp of most recent outbound contact attempt |
| contact_attempts_count | int | 0 | Total outbound contact attempts for this lead |
| recently_contacted | bool | false | True if outbound attempt within last 72 hours |

**Cooldown rule:** `recently_contacted = true` when there exists at least one outbound contact attempt (`direction = 'outbound'`) with `created_at` within the last 72 hours. This is a marker, not a filter — leads remain visible in all surfaces.

**No breaking changes:** All fields have defaults. Existing consumers that don't read these fields are unaffected. The followup_bridge ignores unknown payload fields.
